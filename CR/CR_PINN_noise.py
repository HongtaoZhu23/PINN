...
for epoch in range(num_epochs):
    ...
    if epoch % 10 == 0:
        ...
        print(f'Epoch {epoch}, Total Loss: {total_loss:.3e}, REG Loss: {loss1:.3e}, EQ1 Loss: {loss2:.3e}, EQ2 Loss: {loss3:.3e}, Time: {elapsed:.3f}, RunningTime: {running_time:.3f}')
        start_time = time.time()
...
torch.save(model.state_dict(), 'CR_PINN_n0.01.pth')
model.load_state_dict(torch.load('CR_PINN_n0.01.pth'))
...
mre_x5 = np.mean(np.abs((Y_test.squeeze() - Y_preds) / Y_test.squeeze()))
mre_x4 = np.mean(np.abs((x2_test.squeeze() - x2_preds) / x2_test.squeeze()))

print(f'Mean Relative Error (x2): {mre_x5:.3%}')
print(f'Mean Relative Error (x1): {mre_x4:.3%}')
